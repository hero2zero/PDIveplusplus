...
- **DNS resolution cached across phases**: `validate_targets` now returns resolved IPs alongside the validated target list. The cache is stored in `ScannerConfig.resolved_ips` and reused during the metadata lookup phase, eliminating duplicate DNS queries.
...
- **Default port scanning upgraded from ~50 common ports to top 1000 ports**
  - Built-in TCP scanner now scans top 1000 ports by default (based on nmap frequency ranking)
...
- Updated `port_scan()` method to parse and use TOP_1000_PORTS
...
- Warning message when scanning all ports with the built-in scanner (due to longer scan time)
...
[1.8.0]: https://github.com/yourusername/PDIveplusplus/compare/v1.7.6...v1.8.0
